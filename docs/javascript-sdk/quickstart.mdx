...
<Steps>
  ...
  <Step title="Start a Local Server">
    To serve your HTML file, you can start a local web server in the directory containing `index.html`:
    <CodeGroup>
      ```bash Python
...
- **Obtaining a Session Token:** Your API key was exchanged for a temporary session token that enables the avatar connection.
- **Establishing a WebRTC Stream:** The TruGen SDK established a real-time WebRTC video/audio stream with the TruGen server.
