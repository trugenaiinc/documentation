...
description: "Real-time video analysis for monitoring, proctoring, and engagement tracking: powered by Hawkeye-1."
---
...
<Note>This feature is available only for enterprise customers.</Note>
...
The system connects to a live video session, processes video frames in real time, and pushes results instantly to dashboards, apps, or external systems. The same events are also published directly into the LiveKit room on the `hawkeye-events` data-channel topic, so in-room clients can react without running a callback server.
...
Events reach you over a WebSocket callback and the LiveKit data channel (`hawkeye-events` topic), so both backend services and in-room clients can act in real time. See [Event Delivery](/docs/agents/vision/start-session#event-delivery) for the exact formats.
...
<Card title="Foundation Models" icon="brain" href="/docs/models/overview">
  Learn about the Huma family, the models powering TruGen's real-time avatars.
</Card>
