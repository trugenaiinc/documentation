...
| `callback_url`  | ➖       | WebSocket URL to receive real-time alerts. Every event is also published to the LiveKit room's data channel, so this is optional: omit it if you only intend to consume the data channel or poll the [Get Session Status](/docs/agents/vision/session-status) endpoint. |
...
The `type` field identifies the event kind, and `payload` contains the same fields as the WebSocket callback. Data-channel delivery is best-effort: if a publish fails, Hawkeye logs the error and carries on, without interrupting analysis or the WebSocket callback.
